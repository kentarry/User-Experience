...
import {
  ...
  Code, Trophy, FolderInput, Eye, EyeOff
} from 'lucide-react';
...
const App = () => {
  ...
  const analyzeImage = async (index, base64Image) => {
    ...
    if (!apiKey) { showModal("請先設定 VITE_GEMINI_API_KEY 環境變數", "API Key 缺失", true); return; }
  ...
  const renderIssueRow = (item, idx, colorClass) => (
    <tr key={idx} className="hover:bg-slate-750">
      <td className={`px-4 py-3 text-center ${colorClass} font-mono border-r border-slate-600`}>{item.id}</td>
      ...
      <td className="px-4 py-3 text-slate-300 leading-relaxed whitespace-pre-wrap">
        ...
        {previewMode === 'internal' && item.bestSuggestionRawText && (
        ...
        {previewMode === 'internal' && item.bestSuggester?.name && (
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs uppercase bg-slate-700 text-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 w-16 text-center border-r border-slate-600">人數</th>
                <th className="px-4 py-3 w-16 text-center border-r border-slate-600">編號</th>
                <th className="px-4 py-3">組員優化建議 (Suggestion)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-600 bg-slate-800">
              {issues.map((item, idx) => renderIssueRow(item, idx, colorClass))}
            </tbody>
          </table>
    ...
    <div className="flex h-screen w-full bg-slate-900 text-slate-100 font-sans overflow-hidden relative">
              ...
              <div key={idx} className="flex gap-6 border-b border-slate-700 pb-6 last:border-0 last:pb-0">
                <div className="w-1/2 flex flex-col gap-2">
                  ...
                  <div className="rounded-lg overflow-hidden border border-slate-600 relative group bg-black/40 min-h-[150px] flex items-center justify-center">
                    {item.imageUrl ? (
                      <img src={item.imageUrl} alt="Analysis" className="w-full object-cover" />
                    ) : (
      ...
      <div className="w-[30%] bg-slate-800 border-l border-slate-700 flex flex-col h-full shadow-2xl z-10">
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-900">
          <button onClick={handleExportHtml} className="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-900/20">
            <Download size={18} /> 匯出 HTML 報告
          </button>
        </div>
      </div>
